...
```{r beta_binom_stan_fit_example_tidy_hpd}
tidy(fit, interval_type = "HPD", digits = 2) # HPD intervals require coda
# tidy(fit) will give credible intervals using the quantile method
```
...
posterior_summaries <- cbind(data_frame(day = 1, term = colnames(fit$posterior_simulations)), tidy(fit))
```
...
for (day in 2:14) {
  ...
  posterior_summaries <- rbind(posterior_summaries,
                               cbind(data_frame(day = day,
                                                term = colnames(fit$posterior_simulations)),
                                     tidy(fit)))
}
