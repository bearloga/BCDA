...
As before, we have a null hypothesis that outcome is independent of group. This time, we are going to use the Bayes factor (computed with the **LearnBayes** package) and interpret it using Kass and Raftery's (1995) suggestion.
...
Another way to do this is to fit a log-linear model to the counts and look at the interaction. This requires the **conting** package.
...
Under the $\chi^2$ discrepancy statistic, the Bayesian p-value of 0.51 does not indicate an inadequate model meaning that predictions of the cell counts are consistent with the observed cell counts.

Positive posterior mean -- 0.1514, 95% C.I. (0.99, 0.22) -- of the interaction parameter indicates that a physician taking aspirin has an increased probability of not having an attack.
...
```{r beta_binom_stan_fit_example_summary_hpd, eval = FALSE}
...
```{r beta_binom_stan_fit, echo = FALSE}
